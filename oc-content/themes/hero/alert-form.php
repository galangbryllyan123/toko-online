...
$(document).ready(function(){
    $(".btn_sub").click(function(){
        $.post('<?php echo osc_base_url(true); ?>', {email:$("#alert_email").val(), userid:$("#alert_userId").val(), alert:$("#alert").val(), page:"ajax", action:"alerts"}, 
            function(data){
                if(data==1) { alert('<?php echo osc_esc_js(__('You have sucessfully subscribed to the alert', 'hero')); ?>'); }
                else if(data==-1) { alert('<?php echo osc_esc_js(__('Invalid email address', 'hero')); ?>'); }
                else { alert('<?php echo osc_esc_js(__('There was a problem with the alert', 'hero')); ?>');
                };
        });
        return false;
    });
...
<div class="alert_form">
    <h4>
        <strong><?php _e("Subscribe to this search", 'hero'); ?></strong>
    </h4>
    <form action="<?php echo osc_base_url(true); ?>" method="post" name="sub_alert" id="sub_alert">
        <fieldset>
            ...
            <button type="submit" class="btn btn-info btn_sub seratus">
                <?php _e("Subscribe now", 'hero'); ?></button>
        </fieldset>
    </form>
</div>