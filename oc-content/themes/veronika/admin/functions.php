...
function veronika_footer() {
  ...
  $text .= '<a target="_blank" href="http://forums.mb-themes.com/"><i class="fa fa-comments"></i> ' . __('Support Forums', 'veronika') . '</a>';
...
function veronika_has_subcategories_special($categories, $deep = 0) {
  ...
  foreach($categories as $c) {
    ...
    if(($c['s_color'] == '' and $_POST['color' .$c['pk_i_id']] == '') or (isset($_POST['color' .$c['pk_i_id']]) and $_POST['color' .$c['pk_i_id']] == '')) { 
      ...
      echo __('Color', 'veronika') . ': ' . ($_POST['color' .$c['pk_i_id']] <> '' ? $_POST['color' .$c['pk_i_id']] : $c['s_color']) . '<span class="show-color" style="background:' . ($_POST['color' .$c['pk_i_id']] <> '' ? $_POST['color' .$c['pk_i_id']] : $c['s_color']) . '"></span>';
    }
