...
function tatiana_backoffice_menu($title = NULL) {
  ...
  if( $title == '') { $title = __('Settings', 'tatiana'); }

  $text  = '<div class="mb-head">';
  $text .= '<div class="mb-head-left">';
  $text .= '<h1>' . $title . '</h1>';
  $text .= '<h2>Tatiana Theme</h2>';
  $text .= '</div>';
  $text .= '<div class="mb-head-right">';
  $text .= '<ul class="mb-menu">';
  $text .= '<li><a href="' . osc_base_url() . 'oc-admin/index.php?page=appearance&action=render&file=oc-content/themes/tatiana/admin/settings.php"><i class="fa fa-wrench"></i><span>' . __('Settings', 'tatiana') . '</span></a></li>';
  $text .= '<li><a href="' . osc_base_url() . 'oc-admin/index.php?page=appearance&action=render&file=oc-content/themes/tatiana/admin/header.php"><i class="fa fa-desktop"></i><span>' . __('Header', 'tatiana') . '</span></a></li>';
  $text .= '</ul>';
  $text .= '</div>';
  $text .= '</div>';
...
function tatiana_footer() {
  $themeInfo = tatiana_theme_info();
  ...
  $text .= '<a target="_blank" href="' . $themeInfo['support_uri'] . '"><i class="fa fa-bug"></i> ' . __('Report Bug', 'tatiana') . '</a>';
  $text .= '<a target="_blank" href="http://forums.mb-themes.com/"><i class="fa fa-comments"></i> ' . __('Support Forums', 'tatiana') . '</a>';
  $text .= '<a target="_blank" class="mb-last" href="mailto:info@mb-themes.com"><i class="fa fa-envelope"></i> ' . __('Contact Us', 'tatiana') . '</a>';
...
// List of categories
function tatiana_has_subcategories_special($categories, $deep = 0) {
  $i = 1;
  foreach($categories as $c) {
    ...
    echo '<div class="mb-col-3"><a class="add_img" id="small' . $c['pk_i_id'] . '" href="#">' . __('Add small image', 'tatiana') . '</a></div>';

    if (file_exists(osc_themes_path() . osc_current_web_theme() . '/images/large_cat/' . $c['pk_i_id'] . '.png')) { 
    ...
    echo '<div class="mb-col-3"><a class="add_img" id="large' . $c['pk_i_id'] . '" href="#">' . __('Add large image', 'tatiana') . '</a></div>';
    //echo '<div class="mb-col-1_1_2 fa-icon"><a class="add_fa" id="fa-icon' . $c['pk_i_id'] . '" href="#" title="To remove icon click on link and leave input empty.">' . __('Add / remove icon', 'tatiana') . '</a>';
    ...
    if(isset($c['categories']) && is_array($c['categories']) && !empty($c['categories'])) {
      tatiana_has_subcategories_special($c['categories'], $deep+1);
    }
