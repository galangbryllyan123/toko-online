...
<body id="body-user-dashboard" class="body-ua">
  ...
  <?php
    ...
    $query = "SELECT (year(s.dt_date)*100 + month(s.dt_date)) as yearmonth, sum(s.i_num_views) as views, sum(s.i_num_premium_views) as premium_views, sum(s.i_num_phone_clicks) as phone_clicks FROM {$db_prefix}t_item_stats s, {$db_prefix}t_item i WHERE s.fk_i_item_id = i.pk_i_id AND i.fk_i_user_id = " . osc_user_id() . " AND year(s.dt_date) >= " . $limit_year . " AND month(s.dt_date) >= " . $limit_month . " GROUP BY (year(s.dt_date)*100 + month(s.dt_date)) ORDER BY (year(s.dt_date)*100 + month(s.dt_date)) asc;";
