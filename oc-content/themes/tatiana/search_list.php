  ...
  <?php while(osc_has_premiums())  { ?>
    <tr class="<?php echo $class; ?> premium" <?php if(Params::getParam('new_window')) { ?>onclick="window.open('<?php echo osc_premium_url();?>', '_blank');"<?php } else { ?>onclick="location.href='<?php echo osc_premium_url();?>';"<?php } ?>>
      ...
      <td class="text <?php if(!osc_images_enabled_at_items()) {?>no-pic<?php } ?>">      
        <h3><a href="<?php if(Params::getParam('new_window')) { echo 'javascript:void(0)'; } else { echo osc_premium_url();} ?>" title="<?php echo osc_premium_title(); ?>"><?php echo ucfirst( osc_highlight( strip_tags( osc_premium_title() ), 120 ) ); ?></a></h3>
        ...
        <div class="zoznam_views"><div class="icon-count-photos"></div></span><?php echo osc_count_premium_resources();?>x</span></div>
        <div class="zoznam_desc"><?php echo osc_highlight(strip_tags( osc_premium_description() ), 350 ); ?></div>
...
<div class="regular-offer round2">
  ...
  <div class="results"><?php echo osc_default_results_per_page_at_search()*(osc_search_page())+1;?> - <?php echo osc_default_results_per_page_at_search()*(osc_search_page()+1)+osc_count_items()-osc_default_results_per_page_at_search();?> <?php echo ' ' . __('of', 'tatiana') . ' '; ?> <?php echo osc_search_total_items() ?></div>
</div>
...
<tbody>
  ...
  <?php while(osc_has_items())  { if (1==1 /* !osc_item_is_premium() */) {  ?>
    <tr class="<?php echo $class; ?>" <?php if(Params::getParam('new_window')) { ?>onclick="window.open('<?php echo osc_item_url();?>', '_blank');"<?php } else { ?>onclick="location.href='<?php echo osc_item_url();?>';"<?php } ?>>
      ...
      <td class="text <?php if(!osc_images_enabled_at_items()) {?>no-pic<?php } ?>">      
        <h3><a href="<?php if(Params::getParam('new_window')) { echo 'javascript:void(0)'; } else { echo osc_item_url();} ?>" title="<?php echo osc_item_title(); ?>"><?php echo ucfirst( osc_highlight( strip_tags( osc_item_title() ), 120 ) ); ?></a></h3>
