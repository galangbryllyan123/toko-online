...
{
    $params = array("id" => mdh_thread_id(), "status" => mdh_status_id());

    if (Params::getParam("secret") != "") {
        $params["secret"] = Params::getParam("secret");
    }

    if (Params::getParam("email")) {
        $params["email"] = Params::getParam("email");
    }
    //return mdh_messenger_ajax_url() . "&do=status_change&thread_id=" . mdh_thread_id() . "&new_status=" . mdh_status_id();
    return osc_route_url(mdh_current_plugin_name() . "_thread_status", $params);
}

/**
 * Messenger change title URL.
 * @return a string.
 * @since 2.0.0
 */
function mdh_messenger_title_url() {
    return osc_route_url(mdh_current_plugin_name() . "_thread_title");
}
...
function mdh_thread_status_text($default=null) {
    if(mdh_thread_has_status()) {
        return mdh_thread()->getStatus()->getText();
    } else {
        if(! is_null($default)) {
            return $default;
        }
...
function mdh_thread_has_resources()
{
    return mdh_helpers_loop("mdh_thread_resources", "mdh_thread_resource");
}

function mdh_thread_resource()
{
    $e = __get("mdh_thread_resource");
    if(! $e) {
        return "";
    }
    return $e;
...
function mdh_thread_resources_count() {
    return mdh_helpers_count("mdh_thread_resources");
}

/**
 * Return true if the user can do the block action
 * @return boolean
 *
 * @since 2.0.1
 */
function mdh_thread_can_block() {
    return osc_get_preference("enable_block_user", "plugin_madhouse_messenger")  && osc_is_web_user_logged_in() && !mdh_thread()->isGroup() && !mdh_thread()->isAlone() && !mdh_thread()->getOther()->isDeleted();
}
 ...
 * @since 1.20
 */
function mdh_message_delete_url($secret = "", $email = "")
{
    return mdh_message()->getDeleteURL($secret, $email);
}
...
/**
 * Iterates through blocked users and exports each one to View at each call.
 * @return true if there's remaining threads in the loop, false otherwise.
 * @since 1.50
 */
function mdh_messenger_has_blocked_users()
{
    return mdh_helpers_loop("mdh_blocked_users", "mdh_blocked_user");
}

/**
 * Returns the collection of the exported blocked users.
 * @return an array of Madhouse_Messenger_BlockedUser objects.
 * @since 1.50
 */
function mdh_messenger_blocked_users()
{
    return View::newInstance()->_get("mdh_blocked_users");
}

/**
 * Returns the current message.
 * @return a Madhouse_Messenger_BlockedUser object
 * @throws Exception if no current blocked user is found.
 * @since 1.20
 */
function mdh_messenger_blocked_user()
{
    $e = View::newInstance()->_get("mdh_blocked_user");
    if(! $e) {
        return false;
    }
    return $e;
