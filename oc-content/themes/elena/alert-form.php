...
$(document).ready(function(){
    $(".sub_button").click(function(){
        $.post('<?php echo osc_base_url(true); ?>', {email:$("#alert_email").val(), userid:$("#alert_userId").val(), alert:$("#alert").val(), page:"ajax", action:"alerts"}, 
            function(data){
                if(data==1) { alert('<?php echo osc_esc_js(__('You have sucessfully subscribed to the alert', 'elena')); ?>'); }
                else if(data==-1) { alert('<?php echo osc_esc_js(__('Invalid email address', 'elena')); ?>'); }
                else { alert('<?php echo osc_esc_js(__('There was a problem with the alert', 'elena')); ?>');
                };
        });
        return false;
    });
    ...
    <form action="<?php echo osc_base_url(true); ?>" method="post" name="sub_alert" id="sub_alert">
        <fieldset><div class="box">
            ...
            <?php }; ?>    </div>
            <button style="float:left" type="submit" class="sub_button"><?php _e('Subscribe now', 'elena'); ?>!</button>
        </fieldset>
    </form>
