<?php
define('VERONIKA_THEME_VERSION', '1.1.1');
...
function veronika_check_sold(){
  ...
  if($status <> '' && $id <> '' && $id > 0) {
    ...
    if( $secret == $item['s_secret'] ) {
      Item::newInstance()->dao->update(DB_TABLE_PREFIX.'t_item', array('i_sold' => $status), array('pk_i_id' => $item['pk_i_id']));
...
function veronika_category_loop( $parent_id = NULL, $parent_color = NULL ) {
  ...
  if($parent_id <> '' && $parent_id > 0) {
    $categories = Category::newInstance()->findSubcategories( $parent_id );
  } else {
    $parent_id = 0;
  ...
  foreach( $categories as $c ) {
    ...
    $subcategories = Category::newInstance()->findSubcategories( $c['pk_i_id'] );
...
function veronika_flat_category_select(){  
  $root = Category::newInstance()->findRootCategories();
...
function veronika_user_menu() {
  ...
  $options = array();
  $options[] = array('name' => __('Active'), 'url' => osc_user_list_items_url() . $s_active, 'class' => 'opt_active_items', 'icon' => 'fa-check-square-o', 'section' => 1, 'count' => $c_active, 'is_active' => $yes_active);
  $options[] = array('name' => __('Not Validated'), 'url' => osc_user_list_items_url() . $s_pending, 'class' => 'opt_not_validated_items', 'icon' => 'fa-stack-overflow', 'section' => 1, 'count' => $c_pending, 'is_active' => $yes_pending);
  $options[] = array('name' => __('Expired'), 'url' => osc_user_list_items_url() . $s_expired, 'class' => 'opt_expired_items', 'icon' => 'fa-times-circle', 'section' => 1, 'count' => $c_expired, 'is_active' => $yes_expired);
  $options[] = array('name' => __('Dashboard'), 'url' => osc_user_dashboard_url(), 'class' => 'opt_dashboard', 'icon' => 'fa-dashboard', 'section' => 2);
  $options[] = array('name' => __('Alerts'), 'url' => osc_user_alerts_url(), 'class' => 'opt_alerts', 'icon' => 'fa-bullhorn', 'section' => 2);
  $options[] = array('name' => __('My profile'), 'url' => osc_user_profile_url(), 'class' => 'opt_account', 'icon' => 'fa-file-text-o', 'section' => 2);
  $options[] = array('name' => __('Public Profile'), 'url' => osc_user_public_profile_url(), 'class' => 'opt_publicprofile', 'icon' => 'fa-picture-o', 'section' => 2);
  $options[] = array('name' => __('Logout'), 'url' => osc_user_logout_url(), 'class' => 'opt_logout', 'icon' => 'fa-sign-out', 'section' => 3);
...
function veronika_extra_add_admin( $catId = null, $item_id = null ){
  ...
  if (strpos($current_url, $admin_url) !== false) {
    if($item_id > 0) {
      ...
      echo '<div class="controls"><input type="text" name="sPhone" id="sPhone" value="' . $item['s_phone'] . '" /></div>';
      echo '</div>';

      echo '<div class="control-group">';
      echo '<label class="control-label" for="sSold">' . __('Item Sold', 'veronika') . '</label>';
      echo '<div class="controls"><input type="checkbox" name="sSold" id="sSold" ' . ($item['i_sold'] == 1 ? 'checked' : '') . ' /></div>';
      echo '</div>';
    }
  }
}
...
function veronika_simple_currency() {
  ...
  $currency = $currency <> '' ? $currency : osc_get_preference('currency');
...
function mb_filter_extend() {
  ...
  if(Params::getParam('sTransaction') <> '') {
    Search::newInstance()->addConditions(sprintf("%st_item.i_transaction = %d", DB_TABLE_PREFIX, Params::getParam('sTransaction')));
  }


  // SEARCH - CONDITION
  if(Params::getParam('sCondition') <> '') {
    Search::newInstance()->addConditions(sprintf("%st_item.i_condition = %d", DB_TABLE_PREFIX, Params::getParam('sCondition')));
  }
...
function theme_veronika_actions_admin() {
  ...
  if( Params::getParam('veronika_banner') == 'done' ) {
    ...
    osc_set_preference('banner_home', stripslashes(Params::getParam('banner_home', false, false)), 'veronika_theme');
    osc_set_preference('banner_search', stripslashes(Params::getParam('banner_search', false, false)), 'veronika_theme');
    osc_set_preference('banner_item', stripslashes(Params::getParam('banner_item', false, false)), 'veronika_theme');
...
if( !function_exists('veronika_theme_install') ) {
  ...
  function veronika_theme_install() {
    ...
    osc_set_preference('banner_home', '', 'veronika_theme');
    osc_set_preference('banner_search', '', 'veronika_theme');
    osc_set_preference('banner_item', '', 'veronika_theme');
    ...
    veronika_add_color_col();  // add s_color column to database if does not exists
...
function veronika_default_colors() {
  $colors = array(1 => '#DC143C', 2 => '#00bcd4', 3 => '#00BCD4', 4 => '#FDE74C', 5 => '#8BC34A', 6 => '#D32F2F', 7 => '#2196F3', 8 => '#777', 999 => '#DC143C');
  return $colors;
}
...
function veronika_increase_clicks($itemId, $itemUserId = NULL) {
  if($itemId > 0) {
    if($itemUserId == '' || ($itemUserId <> '' && $itemUserId <> osc_logged_user_id())) {
      $sql = 'INSERT INTO '.DB_TABLE_PREFIX.'t_item_stats (fk_i_item_id, dt_date, i_num_phone_clicks) VALUES ('.$itemId.', \''.date('Y-m-d H:i:s').'\',1) ON DUPLICATE KEY UPDATE  i_num_phone_clicks = i_num_phone_clicks + 1 ';
      return ItemStats::newInstance()->dao->query($sql);
    }
  }
}
...
function veronika_admin_menu_fix(){
  ...
  echo 'body.compact #menu_veronika_menu.current:after {content:"";display:block;width:6px;height:6px;border-radius:10px;box-shadow:1px 1px 3px rgba(0,0,0,0.1);position:absolute;left:3px;bottom:3px;background:#CC6600}' . PHP_EOL;
...
function veronika_phone_clicks( $item_id ) {
  if( $item_id <> '' ) {
    ...
    $query = "SELECT sum(coalesce(i_num_phone_clicks, 0)) as phone_clicks FROM {$db_prefix}t_item_stats s WHERE fk_i_item_id = " . $item_id . ";";
