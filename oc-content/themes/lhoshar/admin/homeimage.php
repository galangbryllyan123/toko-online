...
<h2 class="render-title"><?php _e('Banner Image', 'lhoshar'); ?></h2>
<?php
    $logo_prefence = osc_get_preference('homeimage', 'lhoshar');
?>
<?php if( is_writable( osc_uploads_path()) ) { ?>
    <?php if($logo_prefence) { ?>
        <h3 class="render-title"><?php _e('Preview', 'lhoshar') ?></h3>
        <img style="max-width:100%;" border="0" alt="<?php echo osc_esc_html( osc_page_title() ); ?>" src="<?php echo lhoshar_homeimage_url().'?'.filemtime(osc_uploads_path() . osc_get_preference('homeimage','lhoshar'));?>" />
