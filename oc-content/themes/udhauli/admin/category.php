...
<h2 class="render-title"><?php _e('Category Icon settings', 'udhauli'); ?></h2>

<form action="<?php echo osc_admin_render_theme_url('oc-content/themes/udhauli/admin/category.php'); ?>" method="post" class="nocsrf">
    <input type="hidden" name="action_specific" value="icons" />
    <fieldset>
        <div class="form-row">
            <p>
                <?php _e('Choose your icons form fontawesome website at', 'udhauli');?>
                <a target="_blank" href="http://fontawesome.io/cheatsheet/">http://fontawesome.io/cheatsheet/</a>
            </p>
        </div>
        <div class="form-horizontal">
        <?php  $icons = unserialize(osc_get_preference('icons', 'udhauli')); while ( osc_has_categories() ) {?>
