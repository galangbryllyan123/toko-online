...
<?php
    $js_lang = array(
        'delete' => __('Delete', 'udhauli'),
        'cancel' => __('Cancel', 'udhauli')
    );
...
<!-- favicon -->
<link rel="shortcut icon" href="<?php echo udhauli_favicon_url(); ?>" type="image/x-icon" />
<!-- /favicon -->
...
<script type="text/javascript">
    var udhauli = window.udhauli || {};
    udhauli.base_url = '<?php echo osc_base_url(true); ?>';
    udhauli.langs = <?php echo json_encode($js_lang); ?>;
    udhauli.fancybox_prev = '<?php echo osc_esc_js( __('Previous image','udhauli')) ?>';
    udhauli.fancybox_next = '<?php echo osc_esc_js( __('Next image','udhauli')) ?>';
    udhauli.fancybox_closeBtn = '<?php echo osc_esc_js( __('Close','udhauli')) ?>';
</script>
...
<?php if(osc_get_preference('rtl_view', 'udhauli') == "1") { ?>
