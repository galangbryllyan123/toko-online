...
{
    public function filter($filters, $rootKey = null)
    {
        $sql = "";
        if (isset($filters["user"]) && isset($filters["label"])) {
            $sql .= $this->filterOnLabels($filters["user"], $filters["label"], $rootKey) . " ";
        }


        if (isset($filters["user"])) {
            $sql .= $this->filterOnUser(
                $filters["user"],
        ...
        if (isset($filters["item"])) {
            $sql .= (!isset($filters["user"]))? "WHERE " : "AND ";
            $sql .= $this->filterOnItem($filters["item"]);
        }
    ...
    public function filterOnItem($itemId)
    {
        return sprintf("t.item_id = %d ", $itemId);
    }
