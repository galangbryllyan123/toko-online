...
<p><?php _e('The preferred size of the favicon is 16x16 pixels.', 'wayst'); ?></p>

<?php
    $logo_prefence = osc_get_preference('favicon', 'wayst');
?>
<?php if( is_writable( osc_uploads_path()) ) { ?>
    <?php if($logo_prefence) { ?>
        <h3 class="render-title"><?php _e('Preview', 'wayst') ?></h3>
        <img style="max-width:100%;" border="0" src="<?php echo wayst_favicon_url().'?'.filemtime(osc_uploads_path() . osc_get_preference('favicon','wayst'));?>" />
